...
contract LiskGarden {
    ...
    event PlantWatered(uint256 indexed plantId, uint256 newWaterLevel);
    ...
    function calculateWaterLevel(uint256 plantId) public view returns (uint256) {
        // TODO: Implement
        Plant memory thePlant = plants[plantId];
        if (!thePlant.exists || thePlant.isDead){
            return 0;
        }
        uint256 timeSinceWatered = block.timestamp - thePlant.lastWatered;
        ...
        if (waterLost >= thePlant.waterLevel){
            return 0;
        }
        return thePlant.waterLevel - waterLost;
    }
    ...
    function updateWaterLevel(uint256 plantId) internal {
        // TODO: Implement
        Plant storage thePlant = plants[plantId];
        uint256 currentWater = calculateWaterLevel(plantId);
        thePlant.waterLevel = currentWater;
        if (currentWater == 0 && !thePlant.isDead){
            thePlant.isDead = true;
    ...
    function waterPlant(uint256 plantId) external {
        // TODO: Implement
        Plant storage thePlant = plants[plantId];
        require(thePlant.exists, "The Plant doesnt exist");
        require(thePlant.owner == msg.sender, "This plant not yours");
        require(!thePlant.isDead, "The Plant is dead");
        thePlant.waterLevel = 100;
        thePlant.lastWatered = block.timestamp;
        emit PlantWatered(plantId, thePlant.waterLevel);
    ...
    function updatePlantStage(uint256 plantId) public {
        // TODO: Implement
        Plant storage thePlant = plants[plantId];
        GrowthStage oldStage = thePlant.stage;
        require(thePlant.exists, "The Plant doesnt exist");
        updateWaterLevel(plantId);
        if (thePlant.isDead){
            return ;
        }
        uint256 timeSincePlanted = block.timestamp - thePlant.lastWatered;
        if(timeSincePlanted >= 3 minutes){
            thePlant.stage = GrowthStage.BLOOMING;
        } 
        else if(timeSincePlanted >= 2 minutes){
            thePlant.stage = GrowthStage.GROWING;
        } 
        else if(timeSincePlanted >= 1 minutes){
            thePlant.stage = GrowthStage.SPROUT;
        } 
        if (thePlant.stage != oldStage){
            emit StageAdvanced(plantId, thePlant.stage);
        }
    }
    ...
    function harvestPlant(uint256 plantId) external {
        ...
        Plant storage thePlant = plants[plantId];
        require(thePlant.exists, "The Plant doesnt exist");
        require(msg.sender == thePlant.owner, "This plant not yours");
        require(!thePlant.isDead, "The Plant is dead");
        updatePlantStage(plantId);
        require(thePlant.stage == GrowthStage.BLOOMING, "The stage doesnt macth");
        thePlant.exists = false;
